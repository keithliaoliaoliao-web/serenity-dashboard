...
def load_tweets_with_fallback(filepath):
    tweets = []
...
def main():
    ...
    raw_tweets = load_tweets_with_fallback(TWEETS_FILE)
    ...
    thesis_database = {}
    print(f"🧠 正在分析 {len(ticker_tweets_map)} 檔個股論點...", flush=True)
    ...
    print(f"✅ AI 個股論點庫生成完畢，已存入 {OUTPUT_THESIS_FILE} (收錄 {len(thesis_database)} 檔)", flush=True)
